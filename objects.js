...
const results = getNamesOfAdults(people);
console.log(results);
...
function groupCategory(products) {
    ...
    products.forEach(product => {
        if (thatgroups[product.category]) {
            thatgroups[product.category].push(product);
        } else {
            thatgroups[product.category] = [product];
        }
    });
    return thatgroups;
}
...
//       if(people[item].age>=18){
//         result.push(people[item].name);
//       }
      
//      }
//      return result
//      }
//      const people=[{name:'Alice',age:17},{name:'Eunice',age:22},{name:'Charle',age:14},{name:'Max',age:19}];
//      const reult = getNamesOfpeople(people);
//      getNamesOfpeople()
